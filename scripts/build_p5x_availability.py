#!/usr/bin/env python3
"""Bake parallel availability records into P5X catalog and URL lists."""
...
DEFAULT_ROOT = ROOT / "docs" / "data" / "p5x"
GENERATED_BY = "scripts/build_p5x_availability.py"
...
def reslist_probe(row: dict[str, Any], checked_at: str) -> ProbeResult:
    status = int_value(row.get("status"))
    url = str(row.get("reslist_url") or row.get("url") or f"p5x-reslist://{row.get('version') or 'unknown'}")
    ok = 200 <= status < 400
    return {
        "url": url,
...
def apply_object_availability(items: list[dict[str, Any]], checked_at: str, adapter: P5xAvailabilityAdapter) -> None:
    for item in items:
...
def main() -> None:
    ...
    print("P5X availability migration")

const state = {
  ...
  androidIndex: null,
  aethergazerResourceIndex: null,
  aethergazerResourceVersions: {},
  aethergazerResourceVersionPromises: new Map(),
  aethergazerResourceEntries: new Map(),
  aethergazerResourcePath: "",
  aethergazerResourceExpandedFile: "",
  aethergazerResourceVisible: 150,
...
const allGames = () => {
  const baseGames = [
    ...
    ...(state.p5xCatalog?.game ? [state.p5xCatalog.game] : []),
...
const p5xVersions = () => (state.p5xCatalog?.versions || []).filter((item) => item.status === 200 && item.full);
const p5xVersion = () => state.p5xCatalog?.versions?.find((item) => item.version === state.version);
const p5xFiles = () => p5xVersion()?.[state.mode];
const resListCatalog = () => isP5x() ? state.p5xCatalog : isTof() ? state.tofCatalog : state.nteCatalog;
const resListVersions = () => isP5x() ? p5xVersions() : isTof() ? tofVersions() : nteVersions();
const resListVersion = () => isP5x() ? p5xVersion() : isTof() ? tofVersion() : nteVersion();
const resListFiles = () => isP5x() ? p5xFiles() : isTof() ? tofFiles() : nteFiles();
const resListGameLabel = () => isP5x() ? "P5X" : isTof() ? "TOF" : "NTE";
const resListSourceLabel = () => isP5x() ? "P5X 官方启动器 ResList" : isTof() ? "幻塔官方启动器 ResList" : "异环官方启动器 ResList";
...
const hasAethergazerResources = () => state.gameId === "aethergazer" && (state.aethergazerResourceIndex?.versions || []).length > 0;
...
const currentGameSyncInfo = () => {
  ...
  if (isTof() || isP5x()) {
    const latest = latestByVersion(resListVersions());
    const catalog = resListCatalog();
    return {
      game,
      source: resListSourceLabel(),
      checked: catalog?.last_checked_at || catalog?.generated_at,
...
const commandFor = () => {
  ...
  if (isTof() || isP5x()) {
    const folder = isP5x() ? "p5x" : "tof";
    return `aria2c -c -x16 -s16 -i data/${folder}/url_lists/${state.version}-${state.mode === "patches" ? "patches.patches" : "full.files"}.aria2.txt`;
  }
  if (isEndfield()) {
...
const versionButton = (item) => {
  ...
  if (state.mode === "resources") {
    return `
      <button class="version-row ${item.version === state.version ? "selected" : ""}" type="button" data-version="${item.version}">
        ...
        <span class="caps">
          <span class="cap green">资源清单</span>
  ...
  if (isResListGame()) {
    const isMajorRelease = isTof() || isP5x() || nteIsMajorRelease(item);
    const releaseTitle = isTof() || isP5x() ? "当前官方 ResList 存档版本" : isMajorRelease ? "该系列最早存档版本" : "同系列后续存档版本";
    return `
      <button class="version-row ${item.version === state.version ? "selected" : ""}" type="button" data-version="${item.version}">
...
const renderLinks = () => {
  ...
  if (isResListGame()) {
    ...
    scriptButton.hidden = disabled || state.mode !== "full" || !isNte();
    scriptButton.disabled = disabled || state.mode !== "full" || !isNte();
    return;
  }
...
const loadAethergazerResourceVersion = async (version = state.version) => {
  if (!version) return null;
  if (state.aethergazerResourceVersions?.[version]) return state.aethergazerResourceVersions[version];
  const summary = aethergazerResourceSummaries().find((item) => item.version === version);
  const url = summary?.links?.json;
  if (!url) return null;
  if (!state.aethergazerResourceVersionPromises.has(version)) {
    state.aethergazerResourceVersionPromises.set(version, fetchJson(url));
  }
  const row = await state.aethergazerResourceVersionPromises.get(version);
  state.aethergazerResourceVersions[version] = row;
  return row;
};

const loadAethergazerResourceEntries = async (version = state.version) => {
...
const aethergazerResourceKindLabels = {
  asset: "游戏资源",
  voice_ja: "日语语音",
  voice_zh: "中文语音",
};

const aethergazerResourceItem = (entry, index = 0, total = 0) => {
  const path = entry.path || "";
  return {
    key: path,
    badge: aethergazerResourceKindLabels[entry.kind] || entry.kind || "资源",
...
const nteCdn2Url = (url) => {
  ...
  const mirror = isP5x()
    ? text.replace("https://nsywl-client-dev1.wmupd.com/", "https://nsywl-client-dev2.wmupd.com/")
    : isTof()
...
const nteItem = (entry, index, total) => {
  ...
  if (state.mode === "patches" || state.mode === "androidPatches") {
    ...
    return {
      ...
      remoteName: patch || entry.url.split("/").at(-1),
      size: entry.filesize,
      hash: patch.split(".")[0] || "-",
  ...
  return {
    ...
    remoteName: entry.filename || entry.object,
    size: entry.filesize,
    hash: entry.md5,
    url: availability?.preferred_url || entry.url,
    mirrorUrl: nteCdn2Url(entry.url),
...
const renderNotice = () => {
  ...
  if (state.mode === "legacy") {
    ...
    const catalog = resListCatalog();
    const gameName = isP5x() ? "P5X" : "幻塔";
    notice.innerHTML = `
      <div class="notice-copy">
        <strong>数据来源</strong>
        <span>页面保存由${gameName}官方启动器 CDN 清单解析出的 URL、校验信息与下载索引；该分发链路与异环同属 PatcherSDK ResList 结构。</span>
      </div>
      <div class="source-links">
        <a class="source-link" href="${escapeHtml(resListVersion()?.reslist_url || "#")}" target="_blank" rel="noreferrer">官方 ResList 清单</a>
        <a class="source-link" href="${escapeHtml(catalog?.config?.url || catalog?.config_urls?.[0] || "#")}" target="_blank" rel="noreferrer">官方 config.xml</a>
      </div>
    `;
  } else if (isEndfield()) {
    notice.innerHTML = `

...
| 游戏 | 平台 | 状态 | 版本更新时间 |
| --- | --- | --- | --- |
| Neverness to Everness / 异环 | Windows PC | 已解码并索引到 `1.3.11`（可用 `58` 个 / 已探测 `105` 个） | `2026-08-22 04:00:01 北京时间` |
...
_整个项目的数据刷新时间：`2026-08-23 13:23:07 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-08-23 12:50:51 北京时间`。

| 范围 | 当前进度 |
| --- | --- |
| NTE / 异环 PC | 已索引官方 Windows 清单 `1.0.0` 到 `1.3.11`；`105` 个已探测条目中有 `58` 个可用版本 |
...
| HoYo CN PC 目录 | 已迁移公开 HoyoFiles 元数据：原神 `1.0.0-7.0.0`（`56` 个版本），崩坏：星穹铁道 `1.0.5-4.4.0`（`29` 个版本），绝区零 `0.2.0-3.1.0`（`20` 个版本），崩坏3 `3.5.0-9.0.0`（`56` 个版本） |

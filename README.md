...
| 游戏 | 平台 | 状态 | 版本更新时间 |
| --- | --- | --- | --- |
| Neverness to Everness / 异环 | Windows PC | 已解码并索引到 `1.3.16`（可用 `63` 个 / 已探测 `110` 个） | `2026-09-09 05:00:01 北京时间` |
| Tower of Fantasy / 幻塔 | Windows PC | 官方 PatcherSDK ResList 已解码并索引到 `6.4.2`（`48` 个版本） | `2026-09-24 07:00:03 北京时间` |
| Persona 5: The Phantom X / 女神异闻录：夜幕魅影 | Windows PC | 官方 PatcherSDK ResList 已解码并索引到 `1.0.75`（`26` 个版本） | `2026-09-24 06:31:35 北京时间` |
| Arknights: Endfield / 明日方舟：终末地 | Windows PC | 官方启动器 API 历史与归档镜像已索引到 `1.5.3`（`9` 个版本） | `2026-09-02 06:17:29 北京时间` |
| Arknights / 明日方舟 | Windows PC | 官方启动器包元数据已索引到 `77.0.0`（`1` 个版本） | `未知` |
| Wuthering Waves / 鸣潮 | Windows PC | 官方启动器资源索引与 CDN 镜像已索引到 `3.6.1`（`46` 个版本） | `2026-09-11 10:56:22 北京时间` |
| Genshin Impact / 原神 | Windows PC | HoyoFiles 版本元数据已迁移到 `7.1.0`（`57` 个版本） | `2026-09-12 23:02:49 北京时间` |
| Honkai: Star Rail / 崩坏：星穹铁道 | Windows PC | HoyoFiles 版本元数据已迁移到 `4.6.0`（`31` 个版本） | `2026-09-18 01:56:47 北京时间` |
| Zenless Zone Zero / 绝区零 | Windows PC | HoyoFiles 版本元数据已迁移到 `3.2.0`（`21` 个版本） | `2026-08-22 11:58:30 北京时间` |
| Honkai Impact 3 / 崩坏3 | Windows PC | HoyoFiles 版本元数据已迁移到 `9.1.0`（`57` 个版本） | `2026-09-18 12:20:45 北京时间` |

_整个项目的数据刷新时间：`2026-09-25 09:41:46 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-09-25 09:34:43 北京时间`。

| 范围 | 当前进度 |
| --- | --- |
| NTE / 异环 PC | 已索引官方 Windows 清单 `1.0.0` 到 `1.3.16`；`110` 个已探测条目中有 `63` 个可用版本 |
| Tower of Fantasy / 幻塔 PC | 已索引官方 Windows ResList `5.5.3` 到 `6.4.2`；最新清单包含 `96` 个完整文件与 `660` 个补丁对象 |
| Persona 5: The Phantom X / 女神异闻录：夜幕魅影 PC | 已索引官方 Windows ResList `1.0.34` 到 `1.0.75`；最新清单包含 `3` 个完整文件与 `9` 个补丁对象 |
| Endfield / 终末地 PC | 已导入 `9` 个 CN 启动器历史快照，最新 `1.5.3`；官方签名包 URL 与归档镜像 URL 均已保留 |
| Arknights / 明日方舟 PC | 官方启动器包元数据已索引到 `77.0.0`；最新快照包含 `20` 个包条目 |
| Wuthering Waves / 鸣潮 PC | 已索引 `46` 个 CN 启动器 / resource-index 快照，最新 `3.6.1`；官方索引暴露的文件 URL、CDN 镜像与补丁路由均已保留 |
| HoYo CN PC 目录 | 已迁移公开 HoyoFiles 元数据：原神 `1.0.0-7.1.0`（`57` 个版本），崩坏：星穹铁道 `1.0.5-4.6.0`（`31` 个版本），绝区零 `0.2.0-3.2.0`（`21` 个版本），崩坏3 `3.5.0-9.1.0`（`57` 个版本） |
| Android APK 归档 | 保留 `18` 个游戏的 `290` 条已确认或历史验证过的官方 APK CDN 记录 |
...
| 游戏 | 已索引版本 | 记录 | 备注 |
| --- | --- | --- | --- |
| 深空之眼 / Aether Gazer | `0.285.0` -> `0.307.210` | `7` 个版本桶 / `7` 条记录 | `2` 条可用；`5` 条不可用或历史死链记录 |
| 明日方舟 / Arknights | `1.1.5.0` -> `2.7.7.1` | `23` 个版本桶 / `23` 条记录 | `2` 条可用；`21` 条不可用或历史死链记录 |
| 崩坏学园2 / Houkai Gakuen 2 | `3.4.8` -> `13.3.8` | `33` 个版本桶 / `35` 条记录 | `2` 条可用；`33` 条不可用或历史死链记录 |
| 崩坏3 / Honkai Impact 3rd | `0.9.9` -> `9.1.0` | `56` 个版本桶 / `59` 条记录 | `22` 条可用；`37` 条不可用或历史死链记录 |
| 碧蓝档案 / Blue Archive | `1.8.2` -> `3.0.2` | `4` 个版本桶 / `4` 条记录 | `2` 条可用；`2` 条不可用或历史死链记录 |
| 卡拉比丘 / Calabiyau | `1.1.1.1972` -> `1.1.9.1` | `2` 个版本桶 / `2` 条记录 | `1` 条可用；`1` 条不可用或历史死链记录 |
| 明日方舟：终末地 / Arknights: Endfield | `1.1.8` -> `1.5.3` | `3` 个版本桶 / `3` 条记录 | `1` 条可用；`2` 条不可用或历史死链记录 |
| 少女前线2：追放 / Girls' Frontline 2: Exilium | `4.0.0` | `1` 个版本桶 / `1` 条记录 | `1` 条可用；`0` 条不可用或历史死链记录 |
| 原神 / Genshin Impact | `0.9.3` -> `7.1.0` | `54` 个版本桶 / `54` 条记录 | `45` 条可用；`9` 条不可用或历史死链记录 |
| 崩坏：星穹铁道 / Honkai: Star Rail | `0.90.0` -> `4.5.0` | `30` 个版本桶 / `34` 条记录 | `31` 条可用；`3` 条不可用或历史死链记录 |
| 绝区零 / Zenless Zone Zero | `1.0.0` -> `3.2.0` | `10` 个版本桶 / `10` 条记录 | `10` 条可用；`0` 条不可用或历史死链记录 |
| 异环 / Neverness to Everness | `1.0.2` -> `1.4.0` | `2` 个版本桶 / `2` 条记录 | `1` 条可用；`1` 条不可用或历史死链记录 |
| 女神异闻录：夜幕魅影 / Persona 5: The Phantom X | `0.3.0` -> `1.5.8` | `10` 个版本桶 / `11` 条记录 | `11` 条可用；`0` 条不可用或历史死链记录 |
| 战双帕弥什 / Punishing: Gray Raven | `4.8.0` | `1` 个版本桶 / `1` 条记录 | `1` 条可用；`0` 条不可用或历史死链记录 |
| 重返未来：1999 / Reverse: 1999 | `0.4.0` -> `4.0.0` | `3` 个版本桶 / `3` 条记录 | `1` 条可用；`2` 条不可用或历史死链记录 |
| 尘白禁区 / Snowbreak: Containment Zone | `1.6.0.99` -> `3.6.0.122` | `9` 个版本桶 / `9` 条记录 | `4` 条可用；`5` 条不可用或历史死链记录 |
| 幻塔 / Tower of Fantasy | `4.2` | `1` 个版本桶 / `1` 条记录 | `0` 条可用；`1` 条不可用或历史死链记录 |
| 鸣潮 / Wuthering Waves | `0.7.0` -> `3.6.1` | `31` 个版本桶 / `31` 条记录 | `24` 条可用；`7` 条不可用或历史死链记录 |

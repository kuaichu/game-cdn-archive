...
| 游戏 | 平台 | 状态 | 版本更新时间 |
| --- | --- | --- | --- |
| Neverness to Everness / 异环 | Windows PC | 已解码并索引到 `1.3.13`（可用 `60` 个 / 已探测 `107` 个） | `2026-08-28 04:00:01 北京时间` |
...
| Arknights / 明日方舟 | Windows PC | 官方启动器包元数据已索引到 `76.0.0`（`1` 个版本） | `未知` |
...
_整个项目的数据刷新时间：`2026-09-02 16:53:29 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-09-02 16:46:07 北京时间`。

| 范围 | 当前进度 |
| --- | --- |
| NTE / 异环 PC | 已索引官方 Windows 清单 `1.0.0` 到 `1.3.13`；`107` 个已探测条目中有 `60` 个可用版本 |
...
| Arknights / 明日方舟 PC | 官方启动器包元数据已索引到 `76.0.0`；最新快照包含 `20` 个包条目 |
...
| 明日方舟 / Arknights | `1.1.5.0` -> `2.7.6.1` | `23` 个版本桶 / `23` 条记录 | `2` 条可用；`21` 条不可用或历史死链记录 |

...
| 游戏 | 平台 | 状态 | 版本更新时间 |
| --- | --- | --- | --- |
| Neverness to Everness / 异环 | Windows PC | 已解码并索引到 `1.2.16`（可用 `44` 个 / 已探测 `79` 个） | `2026-07-14 04:00:01 北京时间` |
...
| Wuthering Waves / 鸣潮 | Windows PC | 官方启动器资源索引与 CDN 镜像已索引到 `3.5.2`（`43` 个版本） | `2026-07-16 15:49:59 北京时间` |
...
| Honkai Impact 3 / 崩坏3 | Windows PC | HoyoFiles 版本元数据已迁移到 `8.9.0`（`55` 个版本） | `2026-05-22 11:08:32 北京时间` |

_整个项目的数据刷新时间：`2026-07-21 08:18:53 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-07-21 08:12:37 北京时间`。

| 范围 | 当前进度 |
| --- | --- |
| NTE / 异环 PC | 已索引官方 Windows 清单 `1.0.0` 到 `1.2.16`；`79` 个已探测条目中有 `44` 个可用版本 |
...
| Wuthering Waves / 鸣潮 PC | 已索引 `43` 个 CN 启动器 / resource-index 快照，最新 `3.5.2`；官方索引暴露的文件 URL、CDN 镜像与补丁路由均已保留 |
| HoYo CN PC 目录 | 已迁移公开 HoyoFiles 元数据：原神 `1.0.0-6.7.0`（`55` 个版本），崩坏：星穹铁道 `1.0.5-4.4.0`（`29` 个版本），绝区零 `0.2.0-3.0.0`（`19` 个版本），崩坏3 `3.5.0-8.9.0`（`55` 个版本） |
...
| 游戏 | 已索引版本 | 记录 | 备注 |
| --- | --- | --- | --- |
| 深空之眼 / Aether Gazer | `0.285.0` -> `0.306.120` | `6` 个版本桶 / `6` 条记录 | `2` 条可用；`4` 条不可用或历史死链记录 |
...
| 崩坏3 / Honkai Impact 3rd | `0.9.9` -> `8.9.0` | `55` 个版本桶 / `58` 条记录 | `21` 条可用；`37` 条不可用或历史死链记录 |
| 碧蓝档案 / Blue Archive | `1.8.2` -> `3.0.2` | `4` 个版本桶 / `4` 条记录 | `2` 条可用；`2` 条不可用或历史死链记录 |
| 卡拉比丘 / Calabiyau | `1.1.1.1972` -> `1.1.6.4` | `2` 个版本桶 / `2` 条记录 | `2` 条可用；`0` 条不可用或历史死链记录 |
...
| 重返未来：1999 / Reverse: 1999 | `0.4.0` -> `3.8.0` | `3` 个版本桶 / `3` 条记录 | `1` 条可用；`2` 条不可用或历史死链记录 |

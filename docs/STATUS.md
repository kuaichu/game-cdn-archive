...
| WuWa multi-CDN adapter | partial | Structural metadata availability is done for primary `url` + `urls` candidates. Live multi-CDN probing is implemented for canary `3.4.1`; persistent probe cache infrastructure is done; full-version write remains pending. |

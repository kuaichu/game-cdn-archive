...
| NTE Android resource data | done | `docs/data/nte/android/` uses the NTE adapter for Android post-install ResList resources. APK installers remain in the Android APK archive. |
...
| P5X adapter | done | `adapters/p5x.py` reuses the PatcherSDK ResList interpretation pattern. PC ResList `1.0.74` is indexed under `docs/data/p5x/`; decoded full/patch object URLs are `metadata_inference`. |

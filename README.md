...
| 游戏 | 平台 | 状态 | 版本更新时间 |
| --- | --- | --- | --- |
| Neverness to Everness / 异环 | Windows PC | 已解码并索引到 `1.3.15`（可用 `62` 个 / 已探测 `109` 个） | `2026-09-04 04:00:01 北京时间` |
| Tower of Fantasy / 幻塔 | Windows PC | 官方 PatcherSDK ResList 已解码并索引到 `6.3.3`（`46` 个版本） | `2026-08-27 07:00:01 北京时间` |
...
| Wuthering Waves / 鸣潮 | Windows PC | 官方启动器资源索引与 CDN 镜像已索引到 `3.6.0`（`45` 个版本） | `2026-08-08 05:51:19 北京时间` |
...
_整个项目的数据刷新时间：`2026-09-08 09:34:29 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-09-08 09:14:09 北京时间`。

| 范围 | 当前进度 |
| --- | --- |
| NTE / 异环 PC | 已索引官方 Windows 清单 `1.0.0` 到 `1.3.15`；`109` 个已探测条目中有 `62` 个可用版本 |
| Tower of Fantasy / 幻塔 PC | 已索引官方 Windows ResList `5.5.3` 到 `6.3.3`；最新清单包含 `95` 个完整文件与 `660` 个补丁对象 |
...
| Wuthering Waves / 鸣潮 PC | 已索引 `45` 个 CN 启动器 / resource-index 快照，最新 `3.6.0`；官方索引暴露的文件 URL、CDN 镜像与补丁路由均已保留 |
...
| 游戏 | 已索引版本 | 记录 | 备注 |
| --- | --- | --- | --- |
| 深空之眼 / Aether Gazer | `0.285.0` -> `0.307.210` | `7` 个版本桶 / `7` 条记录 | `5` 条可用；`2` 条不可用或历史死链记录 |
...
| 崩坏3 / Honkai Impact 3rd | `0.9.9` -> `9.0.0` | `56` 个版本桶 / `59` 条记录 | `26` 条可用；`33` 条不可用或历史死链记录 |
| 碧蓝档案 / Blue Archive | `1.8.2` -> `2.1.2` | `3` 个版本桶 / `3` 条记录 | `1` 条可用；`2` 条不可用或历史死链记录 |
| 卡拉比丘 / Calabiyau | `1.1.1.1972` -> `1.1.8.2` | `2` 个版本桶 / `2` 条记录 | `2` 条可用；`0` 条不可用或历史死链记录 |
...
| 绝区零 / Zenless Zone Zero | `1.0.0` -> `3.1.0` | `10` 个版本桶 / `10` 条记录 | `10` 条可用；`0` 条不可用或历史死链记录 |
| 异环 / Neverness to Everness | `1.0.2` -> `1.3.0` | `2` 个版本桶 / `2` 条记录 | `2` 条可用；`0` 条不可用或历史死链记录 |
...
| 鸣潮 / Wuthering Waves | `0.7.0` -> `3.6.0` | `31` 个版本桶 / `31` 条记录 | `24` 条可用；`7` 条不可用或历史死链记录 |

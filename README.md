...
| Honkai Impact 3 / 崩坏3 | Windows PC | HoyoFiles 版本元数据已迁移到 `8.9.0`（`54` 个版本） | `2026-05-22 11:08:32 北京时间` |
...
| HoYo CN PC 目录 | 已迁移公开 HoyoFiles 元数据：原神 `1.0.0-6.7.0`（`55` 个版本），崩坏：星穹铁道 `1.0.5-4.3.0`（`28` 个版本），绝区零 `0.2.0-3.0.0`（`19` 个版本），崩坏3 `3.7.0-8.9.0`（`54` 个版本） |
| Android APK 归档 | 保留 `18` 个游戏的 `279` 条已确认或历史验证过的官方 APK CDN 记录 |
...
| 游戏 | 已索引版本 | 记录 | 备注 |
| --- | --- | --- | --- |
| 深空之眼 / Aether Gazer | `0.285.0` -> `0.306.120` | `5` 个版本桶 / `5` 条记录 | `3` 条可用；`2` 条不可用或历史死链记录 |
| 明日方舟 / Arknights | `2.7.51` -> `2741.0.0` | `21` 个版本桶 / `21` 条记录 | `2` 条可用；`19` 条不可用或历史死链记录 |
...
| 卡拉比丘 / Calabiyau | `1.1.6.4` | `1` 个版本桶 / `1` 条记录 | `1` 条可用；`0` 条不可用或历史死链记录 |

...
| Honkai: Star Rail / 崩坏：星穹铁道 | Windows PC | HoyoFiles 版本元数据已迁移到 `4.5.0`（`30` 个版本） | `2026-08-14 04:27:32 北京时间` |
...
_整个项目的数据刷新时间：`2026-09-24 09:39:59 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-09-24 09:27:44 北京时间`。
...
| HoYo CN PC 目录 | 已迁移公开 HoyoFiles 元数据：原神 `1.0.0-7.1.0`（`57` 个版本），崩坏：星穹铁道 `1.0.5-4.5.0`（`30` 个版本），绝区零 `0.2.0-3.2.0`（`21` 个版本），崩坏3 `3.5.0-9.1.0`（`57` 个版本） |
...
| 卡拉比丘 / Calabiyau | `1.1.1.1972` -> `1.1.9.1` | `2` 个版本桶 / `2` 条记录 | `2` 条可用；`0` 条不可用或历史死链记录 |
...
| 女神异闻录：夜幕魅影 / Persona 5: The Phantom X | `0.3.0` -> `1.5.8` | `10` 个版本桶 / `11` 条记录 | `10` 条可用；`1` 条不可用或历史死链记录 |

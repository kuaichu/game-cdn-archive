...
const wuwaVersions = () => state.wuwaIndex?.versions || [];
const wuwaVersion = () => wuwaVersions().find((item) => item.version === state.version) || null;
const wuwaSummaries = () => wuwaVersions();
...
const versionFamily = (version) => {
  ...
  return isNte() || isEndfield() || isWuwa() ? parts.slice(0, 2).join(".") : `${parts[0]}.x`;
};
...
const wuwaStats = () => {
  const index = state.wuwaIndex;
  const version = wuwaVersion();
  return [
    ["当前版本", index.current_version],
    ["版本总数", `${index.version_count} 个`],
    ["首版本", index.first_version],
    ["末版本", index.latest_version],
    ["当前更新路线", `${version?.patch_routes || 0} 条`],
    ["更新路线体积", fmtBytes(index.total_patch_route_bytes || index.total_patch_chain_bytes || 0)],
    ["当前版本大小", fmtBytes(version?.size || 0)],
    ["索引 MD5", version?.index_file_md5?.slice(0, 16) + "…" || "-"],
  ];
};
...
const renderWuwaVersionChain = () => {
  const versions = wuwaVersions().sort((a, b) => compareVersions(b.version, a.version));
  if (versions.length === 0) {
    $("#fileList").innerHTML = `<div class="empty">没有版本数据</div>`;
    return;
  }
  const rows = versions.map((ver, index) => {
    const majorMinor = ver.version.split(".").slice(0, 2).join(".");
    const indexUrl = ver.resource_index || ver.index_url || "";
    const patchCount = Number(ver.patch_routes || 0);
    return `
      <tr class="wuwa-version-row">
        <td class="wuwa-ver">${escapeHtml(ver.version)}</td>
        <td class="wuwa-family">${majorMinor}</td>
        <td class="wuwa-size">${fmtBytes(ver.size)}</td>
        <td class="wuwa-md5"><code>${escapeHtml(ver.index_file_md5 || "-")}</code></td>
        <td class="wuwa-actions">
          ${patchCount ? `<span class="cap amber">${patchCount} 条更新路线</span>` : ""}
          ${indexUrl ? `<button class="ghost-button" type="button" data-url="${escapeHtml(indexUrl)}" onclick="event.stopPropagation(); navigator.clipboard.writeText(this.dataset.url); showToast('链接已复制')">复制索引 URL</button>` : ""}
        </td>
      </tr>`;
  });
  const totalPatchBytes = state.wuwaIndex.total_patch_route_bytes || state.wuwaIndex.total_patch_chain_bytes || 0;

  $("#fileList").innerHTML = `
    <div class="wuwa-header">
      <strong>国服版本链路 — ${state.wuwaIndex.current_version}</strong>
      <span>共 ${versions.length} 个版本 · 当前更新路线总计 ${fmtBytes(totalPatchBytes)}</span>
    </div>
    <div class="wuwa-table-wrap">
      <table class="wuwa-table">
        <thead><tr>
          <th>版本</th><th>版本族</th><th>文件大小</th><th>索引 MD5</th><th></th>
        </tr></thead>
...
const renderNotice = () => {
  ...
  if (isNte()) {
    ...
    notice.innerHTML = `
      <div class="notice-copy">
        <strong>数据来源</strong>
        <span>数据抓取自鸣潮国服官方启动器 CDN API（Kuro Game Launcher）。完整版本记录为散文件清单；更新路线记录为官方 patchConfig 索引，仅保存 URL 与校验信息，不托管游戏文件。</span>
      </div>
      <div class="source-links">
        <a class="source-link" href="${escapeHtml(state.wuwaIndex.selected_launcher_index || state.wuwaIndex.discovery_url || "#")}" target="_blank" rel="noreferrer">国服官方 CDN 索引</a>
        ${state.wuwaIndex.archive_source ? `<a class="source-link" href="${escapeHtml(state.wuwaIndex.archive_source)}" target="_blank" rel="noreferrer">TomyJan 版本归档</a>` : ""}
        <a class="source-link" href="https://gist.github.com/DynamiByte/d839bf9f671c975b6666d0f6e6634641" target="_blank" rel="noreferrer">API 文档参考</a>
      </div>
    `;
  } else {
    notice.innerHTML = `
...
]).then(async ([nteCatalog, hoyoIndex, endfieldIndex, endfieldVersions, wuwaIndex]) => {

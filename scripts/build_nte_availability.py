#!/usr/bin/env python3
"""Bake parallel availability records into NTE catalog and URL lists."""
...
DEFAULT_ROOT = ROOT / "docs" / "data"
GENERATED_BY = "scripts/build_nte_availability.py"
...
def reslist_probe(row: dict[str, Any], checked_at: str) -> ProbeResult:
    status = int_value(row.get("status"))
    url = str(row.get("reslist_url") or row.get("url") or f"nte-reslist://{row.get('version') or 'unknown'}")
    ok = 200 <= status < 400
    confidence = "high" if status else "low"
    return {
        "url": url,
...
def load_section_items(root: Path, section: dict[str, Any] | None) -> tuple[Path | None, list[dict[str, Any]]]:
    ...
    path = root / rel.relative_to("data")
    if not path.exists():
...
def main() -> None:
    ...
    print("NTE availability migration")

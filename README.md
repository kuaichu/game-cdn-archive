...
| Tower of Fantasy / 幻塔 | Windows PC | 官方 PatcherSDK ResList 已解码并索引到 `6.4.1`（`47` 个版本） | `2026-09-16 06:00:23 北京时间` |
...
_整个项目的数据刷新时间：`2026-09-22 17:29:34 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-09-22 17:17:27 北京时间`。

| 范围 | 当前进度 |
| --- | --- |
| NTE / 异环 PC | 已索引官方 Windows 清单 `1.0.0` 到 `1.3.16`；`110` 个已探测条目中有 `63` 个可用版本 |
| Tower of Fantasy / 幻塔 PC | 已索引官方 Windows ResList `5.5.3` 到 `6.4.1`；最新清单包含 `96` 个完整文件与 `594` 个补丁对象 |
...
| 原神 / Genshin Impact | `0.9.3` -> `7.0.0` | `54` 个版本桶 / `54` 条记录 | `45` 条可用；`9` 条不可用或历史死链记录 |
...
| 异环 / Neverness to Everness | `1.0.2` -> `1.3.0` | `2` 个版本桶 / `2` 条记录 | `1` 条可用；`1` 条不可用或历史死链记录 |
| 女神异闻录：夜幕魅影 / Persona 5: The Phantom X | `0.3.0` -> `1.5.8` | `10` 个版本桶 / `11` 条记录 | `10` 条可用；`1` 条不可用或历史死链记录 |
| 战双帕弥什 / Punishing: Gray Raven | `4.7.0` | `1` 个版本桶 / `2` 条记录 | `2` 条可用；`0` 条不可用或历史死链记录 |
| 重返未来：1999 / Reverse: 1999 | `0.4.0` -> `3.9.0` | `3` 个版本桶 / `3` 条记录 | `1` 条可用；`2` 条不可用或历史死链记录 |

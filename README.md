...
| Genshin Impact / 原神 | Windows PC | HoyoFiles 版本元数据已迁移到 `7.0.0`（`56` 个版本） | `2026-07-31 22:00:00 北京时间` |
...
_整个项目的数据刷新时间：`2026-09-20 09:33:04 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-09-20 09:13:56 北京时间`。
...
| HoYo CN PC 目录 | 已迁移公开 HoyoFiles 元数据：原神 `1.0.0-7.0.0`（`56` 个版本），崩坏：星穹铁道 `1.0.5-4.5.0`（`30` 个版本），绝区零 `0.2.0-3.2.0`（`21` 个版本），崩坏3 `3.5.0-9.0.0`（`56` 个版本） |
...
| 女神异闻录：夜幕魅影 / Persona 5: The Phantom X | `0.3.0` -> `1.5.4` | `10` 个版本桶 / `11` 条记录 | `10` 条可用；`1` 条不可用或历史死链记录 |

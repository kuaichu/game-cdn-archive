...
| 游戏 | 平台 | 状态 | 版本更新时间 |
| --- | --- | --- | --- |
| Neverness to Everness / 异环 | Windows PC | 已解码并索引到 `1.2.15`（可用 `43` 个 / 已探测 `78` 个） | `2026-07-10 08:00:02 北京时间` |
...
_整个项目的数据刷新时间：`2026-07-12 14:53:31 北京时间`。_
...
当前仓库快照来自生成数据，检查时间：`2026-07-12 14:47:40 北京时间`。

| 范围 | 当前进度 |
| --- | --- |
| NTE / 异环 PC | 已索引官方 Windows 清单 `1.0.0` 到 `1.2.15`；`78` 个已探测条目中有 `43` 个可用版本 |
...
| HoYo CN PC 目录 | 已迁移公开 HoyoFiles 元数据：原神 `1.0.0-6.7.0`（`55` 个版本），崩坏：星穹铁道 `1.0.5-4.3.0`（`28` 个版本），绝区零 `0.2.0-3.0.0`（`19` 个版本），崩坏3 `3.5.0-8.9.0`（`55` 个版本） |
